...
class Auditor:
    ...
    def echo_missingness(self, echo_groups_dict):
        """
        Forensic Data Audit: 
    ...
    def transform(self):
        ...
        for category, drugs in self.config.medication_classes.items():
            ...
            self.df[[name_col, dose_col]] = self.df['Medications'].apply(
                lambda x: pd.Series(self._parse_meds(x, drugs, self.config.freq_day))
            )
        ...
        if 'LVEF (categorical)' in self.df.columns:
            self.df['LVEF_Score'] = self.df['LVEF (categorical)'].str.strip().str.title().map(lvef_values)
...
class Visualizer:
    ...
    def patient_nullity(self, target_columns):
        ...
        with sns.axes_style("white"): # Removes the gridlines for this plot only
...
aud = Auditor(df, clinical_config)
# Inject categories dictionary
...
# Inject severity dictionary into constructor
ext = Extractor(df, clinical_config)

# Inject all configuration dictionaries into transform
parsed = ext.transform()

# Inject pathology_patterns into disease_profiler
parsed = ext.disease_profiler()

calc = Calculator(parsed, clinical_config)
# Inject class_weights and max_doses into medication_index
calculated = calc.medication_index()

# This ensures we include the feature engineered columns from the extractor class
aud = Auditor(parsed)

# Inject normal_variables into Statistics constructor
stat = Statistics(calculated, clinical_config)

print('')
# Filter for relevant columns that exist in parsed
viz = Visualizer(calculated)
# Inject echo_vars list
nullity_patterns = viz.patient_nullity(clinical_config.echo_vars)
```
    ...
    Cell In[14], line 7
          4 ext = Extractor(df, clinical_config)
          6 # Inject all configuration dictionaries into transform
    ----> 7 parsed = ext.transform()
          9 # Inject pathology_patterns into disease_profiler
         10 parsed = ext.disease_profiler()
    

    Cell In[13], line 219, in Extractor.transform(self)
        216     dose_col = f"{category}_tdd"
        218     # Use the internal _parse_meds method with injected freq_day
    --> 219     self.df[[name_col, dose_col]] = self.df['Medications'].apply(
        220         lambda x: pd.Series(self._parse_meds(x, drugs, self.config.freq_day))
        221     )
        223 # 5. Left Atrium Size Scoring
        224 self.df['LA_Size_Score'] = 0.0
    

    File c:\Users\Admin\Documents\Git\Statistics\phl_2025\venv_phl\Lib\site-packages\pandas\core\series.py:4943, in Series.apply(self, func, convert_dtype, args, by_row, **kwargs)
       4808 def apply(
       4809     self,
    ...
    File c:\Users\Admin\Documents\Git\Statistics\phl_2025\venv_phl\Lib\site-packages\pandas\core\apply.py:1422, in SeriesApply.apply(self)
       1419     return self.apply_compat()
       1421 # self.func is Callable
    -> 1422 return self.apply_standard()
    

    File c:\Users\Admin\Documents\Git\Statistics\phl_2025\venv_phl\Lib\site-packages\pandas\core\apply.py:1502, in SeriesApply.apply_standard(self)
    ...
    File c:\Users\Admin\Documents\Git\Statistics\phl_2025\venv_phl\Lib\site-packages\pandas\core\base.py:925, in IndexOpsMixin._map_values(self, mapper, na_action, convert)
    ...
    File c:\Users\Admin\Documents\Git\Statistics\phl_2025\venv_phl\Lib\site-packages\pandas\core\algorithms.py:1743, in map_array(arr, mapper, na_action, convert)
    ...
    Cell In[13], line 220, in Extractor.transform.<locals>.<lambda>(x)
        216     dose_col = f"{category}_tdd"
        218     # Use the internal _parse_meds method with injected freq_day
        219     self.df[[name_col, dose_col]] = self.df['Medications'].apply(
    --> 220         lambda x: pd.Series(self._parse_meds(x, drugs, self.config.freq_day))
    ...
    TypeError: Extractor._parse_meds() takes 3 positional arguments but 4 were given
...
# Inject echo_groups dictionary
missings_table = aud.echo_missingness(clinical_config.echo_groups)
...
# Inject list of markers
viz.complexity_markers(['Mixed_Mitral', 'Mixed_Aortic', 'Multi_Valve', 'Has_Shunt', 'AFib_Present', 'PH_Present'])

# Execute
# Inject list of interaction columns
heatmap = viz.complexity_heatmap(['Mixed_Mitral', 'Mixed_Aortic', 'Multi_Valve', 'Has_Shunt', 'AFib_Present', 'PH_Present'])
...
![png](phl_2025_1_files/phl_2025_1_9_0.png)
...
![png](phl_2025_1_files/phl_2025_1_11_0.png)
    


    Analysis Complete: An MBI above 3.25 is the best predictor of patient complexity.
