...
echo_severity = {'trace': 15.0, 'mild': 20.0, 'none': 10.0}

normal_variables = {
    # 'Variable': (Mean, StdDev)
    'MS MG (mmHg)': (1.5, 0.5),   # Minimal flow gradient
    'AO V2 max': (1.1, 0.2),      # Normal laminar flow
    'RVSP': (25.0, 4.0),          # Normal pulmonary pressure range
    'LA_Size_Score': (0.2, 0.1),  # Near-zero but not flat zero
    'LVIDd': (46.0, 4.0)          # Normal LV diameter spread
}
...
echo_missing = [
    'MS MG (mmHg)', 'AO V2 max', 'RVSP', 'LVIDd'
]

imputation_context = ['Age', 'Weight']
```
    ...
    def transform(self):
        ...
        if 'LVEF (categorical)' in self.df.columns:
            ...
            self.df['LVEF_Score'] = self.df['LVEF (categorical)'].str.strip().map(
                {k: v for k, v in lvef_values.items()} # Direct map
            ).fillna(self.df['LVEF (categorical)'].str.strip().str.capitalize().map(lvef_values))
        return self.df
...
class Statistics:
    def __init__(self, dataframe):
        self.df = dataframe.copy()
        self.normal_ranges = normal_variables
    ...
    def impute_normal(self, target_col):
        ...
        if num_missing > 0:
            # Get clinical normal stats or default to (0, 1)
            mean, std = self.normal_ranges.get(target_col, (0, 1))
            
            # Generate the 'Natural' distribution
            simulated_normals = np.random.normal(mean, std, size=num_missing)
            
            # Clip at 0 for measurements that cannot be negative
    ...
    # sns.kdeplot(final_df[param_to_plot].dropna(), label='Original (Pathology-Heavy)', color='red')
    # sns.kdeplot(final_df[f'{param_to_plot}_Imputed'], label='Imputed (Includes "Normal" Skips)', color='green', linestyle='--')
    # plt.title(f"Sensitivity Analysis: {param_to_plot} Shift after MNAR Imputation")
...
![png](phl_2025_1_files/phl_2025_1_4_3.png)
...
![png](phl_2025_1_files/phl_2025_1_4_5.png)
